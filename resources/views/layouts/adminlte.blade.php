...
<div class="wrapper">
  <nav class="main-header navbar navbar-expand navbar-white navbar-light">
    <ul class="navbar-nav">
      ...
      <li class="nav-item d-none d-sm-inline-block"><span class="nav-link text-muted"><i class="fas fa-building mr-1"></i>{{ session('hrms_tenant_name','TalentFlow HRMS') }}</span></li>
    </ul>
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu dropdown-menu-right">
          ...
          @if(session('hrms_role')==='employee')<a href="{{ route('employee.profile') }}" class="dropdown-item"><i class="fas fa-user mr-2"></i>My Profile</a>@endif
          <div class="dropdown-divider"></div>
          <form action="{{ route('logout') }}" method="POST">@csrf<button type="submit" class="dropdown-item text-danger"><i class="fas fa-sign-out-alt mr-2"></i>Logout</button></form>
        </div>
      </li>
    </ul>
  </nav>
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    <a href="#" class="brand-link text-center py-3"><i class="fas fa-users-cog fa-lg mr-2" style="color:#4db8ff"></i><span class="brand-text font-weight-bold" style="color:#fff;font-size:1.1rem">TalentFlow</span></a>
    <div class="sidebar">
      <div class="user-panel mt-3 pb-3 mb-3 d-flex align-items-center" style="border-bottom:1px solid rgba(255,255,255,.1)">
        ...
        <div class="info ml-2"><a href="#" class="d-block" style="color:#fff;font-size:.85rem;font-weight:600">{{ session('hrms_user_name') }}</a><small style="color:#6b8caa">{{ ucfirst(session('hrms_role')) }}</small></div>
      </div>
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu">
          ...
          <li class="nav-item"><a href="{{ route('admin.salary.index') }}" class="nav-link {{ request()->routeIs('admin.salary.*') ? 'active':'' }}"><i class="nav-icon fas fa-sitemap"></i><p>Salary Structures</p></a></li>
          <li class="nav-item"><a href="{{ route('admin.salary.assign') }}" class="nav-link {{ request()->routeIs('admin.salary-assign*') ? 'active':'' }}"><i class="nav-icon fas fa-user-tag"></i><p>Assign Structure</p></a></li>
          <li class="nav-item"><a href="{{ route('admin.salary.offers') }}" class="nav-link {{ request()->routeIs('admin.salary.offer*') ? 'active':'' }}"><i class="nav-icon fas fa-file-contract"></i><p>Offer Letters</p></a></li>
  ...
  <div class="content-wrapper">
    <div class="content-header"><div class="container-fluid"><div class="row mb-2"><div class="col-sm-6"><h1 class="m-0" style="font-size:1.4rem;font-weight:700;color:#1a2942">@yield('page-title','Dashboard')</h1></div><div class="col-sm-6"><ol class="breadcrumb float-sm-right"><li class="breadcrumb-item"><a href="#">Home</a></li><li class="breadcrumb-item active">@yield('breadcrumb','Dashboard')</li></ol></div></div></div></div>
    <div class="content"><div class="container-fluid">
      ...
      @if($errors->any())<div class="alert alert-danger alert-dismissible fade show"><i class="fas fa-exclamation-triangle mr-2"></i>@foreach($errors->all() as $e)<div>{{ $e }}</div>@endforeach<button type="button" class="close" data-dismiss="alert"><span>&times;</span></button></div>@endif
      @yield('content')
    </div></div>
  </div>
...
<script>function updateClock(){const n=new Date();document.getElementById('clock').textContent=n.toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'});}updateClock();setInterval(updateClock,1000);</script>
@stack('scripts')
</body>
</html>
